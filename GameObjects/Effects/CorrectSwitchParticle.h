...
// スイッチ情報
struct CorrectSwitchEmitterInfo
{
	DirectX::SimpleMath::Vector3 position;
	DirectX::SimpleMath::Vector3 scale;
};

class CorrectSwitchParticle
{
...
public:
	...
	// リストを渡す
	void SetEmitters(const std::vector<CorrectSwitchEmitterInfo>& emitters) { m_emitters = emitters; }
...
private:
	...
	// タイプに応じた色を取得
	DirectX::SimpleMath::Color GetColorFromType(SwitchTargetType type);
...
private:
	...
	static const float PARTICLE_START_SCALE;		///< 開始時のスケール
private:
	// メンバ変数 -----------------------------------------------------
	// デバイスリソースへのポインタ
	...
	// 座標リスト
	std::vector<CorrectSwitchEmitterInfo> m_emitters;
};
