	...
	, ANCHOR anchor)
{
	if (!pDR)
	{
		throw std::runtime_error("DeviceResources pointer is null.");
	}
...
void UserInterface::CreateShader()
{
	...
	std::unique_ptr<BinaryFile> VSData = BinaryFile::LoadFile(L"Resources/Shaders/UIVS.cso");
	std::unique_ptr<BinaryFile> GSData = BinaryFile::LoadFile(L"Resources/Shaders/UIGS.cso");
	std::unique_ptr<BinaryFile> PSData = BinaryFile::LoadFile(L"Resources/Shaders/UIPS.cso");
