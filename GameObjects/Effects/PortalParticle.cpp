...
* @return なし
*/
const std::vector<D3D11_INPUT_ELEMENT_DESC> PortalParticle::INPUT_LAYOUT =
{
...
* @return なし
*/
void PortalParticle::LoadTexture(const wchar_t* path)
{
...
* @return なし
*/
void PortalParticle::Create(DX::DeviceResources* pDR)
{
	m_pDR = pDR;
	...
	depthStencilDesc.DepthWriteMask = D3D11_DEPTH_WRITE_MASK_ALL;	// 深度バッファへの書き込みを無効化
	depthStencilDesc.DepthFunc = D3D11_COMPARISON_LESS;				// 深度テストの条件
...
*/
float PortalParticle::RandomFloat(float min, float max)
{
...
* @return なし
*/
void PortalParticle::Update(float elapsedTime)
{
	// 0.1秒ごとに複数のパーティクルを生成
	m_timer += elapsedTime;
	if (m_timer >= 0.1f)
		...
		for (const auto& emitter : m_emitters)
		{
			...
			for (int i = 0; i < particleCount; ++i)
			{
				...
				ParticleUtility pU(
					...
					DirectX::SimpleMath::Color(1, 1, 1, 1),    // 初期カラー
					DirectX::SimpleMath::Color(0, 0, 1, 1)     // 最終カラー
				);
...
* @return なし
*/
void PortalParticle::CreateShader()
{
	ID3D11Device1* device = m_pDR->GetD3DDevice();

	//	コンパイルされたシェーダファイルを読み込み
	std::unique_ptr<BinaryFile> VSData = BinaryFile::LoadFile(L"Resources/Shaders/PortalParticleVS.cso");
	std::unique_ptr<BinaryFile> PSData = BinaryFile::LoadFile(L"Resources/Shaders/PortalParticlePS.cso");
	std::unique_ptr<BinaryFile> GSData = BinaryFile::LoadFile(L"Resources/Shaders/PortalParticleGS.cso");
...
* @return なし
*/
void PortalParticle::Render(DirectX::SimpleMath::Matrix view, DirectX::SimpleMath::Matrix proj)
{
...
* @return なし
*/
void PortalParticle::Billboard(DirectX::SimpleMath::Vector3 target, DirectX::SimpleMath::Vector3 eye, DirectX::SimpleMath::Vector3 up)
{
