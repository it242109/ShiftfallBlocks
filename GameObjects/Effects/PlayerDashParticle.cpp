...
// 定数の定義
const float PlayerDashParticle::EMITTER_RANGE_SCALE = 0.8f;			///< エミッタ―の範囲をどれくらいにするか	
const int PlayerDashParticle::PARTICLE_COUNT = 1;					///< パーティクルの数
const float PlayerDashParticle::MIN_SPARN_RADIUS = 0.8f;			///< パーティクルの最小半径
const float PlayerDashParticle::MAX_SPARN_RADIUS = 1.0f;			///< パーティクルの最大半径
const float PlayerDashParticle::SPAWN_Y_OFFSET = 1.0f;				///< 出現場所Yのオフセット
const float PlayerDashParticle::VELOCITY_HORIZONTAL_WEIGHT = 0.5f;  ///< 横方向の速度の勢い
const float PlayerDashParticle::VELOCITY_UPWARD_BASE = 0.3f;		///< 上方向の基本速度
const float PlayerDashParticle::VELOCITY_SPEED_MIN = 1.5f;		    ///< 最小初速倍率
const float PlayerDashParticle::VELOCITY_SPEED_MAX = 2.0f;			///< 最大初速倍率
const float PlayerDashParticle::PARTICLE_LIFETIME = 1.0f;			///< 生存時間（秒）
const float PlayerDashParticle::PARTICLE_START_SCALE = 0.5f;		///< 開始時のスケール
...
* @return     なし
*/
const std::vector<D3D11_INPUT_ELEMENT_DESC> PlayerDashParticle::INPUT_LAYOUT =
{
...
* @return     なし
*/
PlayerDashParticle::PlayerDashParticle()
	:
	m_pDR(nullptr),
	m_timer(0.0f)
{
...
* @return     なし
*/
void PlayerDashParticle::LoadTexture(const wchar_t* path)
{
	// テクスチャリソースの読み込み
...
* @return     なし
*/
void PlayerDashParticle::Create(DX::DeviceResources* pDR)
{
	m_pDR = pDR;
	...
	//	画像の読み込み
	LoadTexture(L"Resources/Textures/dash.png");
...
*/
float PlayerDashParticle::RandomFloat(float min, float max)
{
...
* @return     なし
*/
void PlayerDashParticle::Update(float elapsedTime)
{
	// 複数のパーティクルを生成
	m_timer += elapsedTime;
	// ランダムエンジンの初期化
	...
	for (const auto& emitter : m_emitters)
	{
		...
		for (int i = 0; i < particleCount; ++i)
		{
			...
			float spawnY = emitter.position.y + (emitter.scale.y * VELOCITY_HORIZONTAL_WEIGHT) - SPAWN_Y_OFFSET;
			...
			velocity.Normalize();
			velocity *= RandomFloat(VELOCITY_SPEED_MIN, PARTICLE_START_SCALE);
...
* @return     なし
*/
void PlayerDashParticle::CreateShader()
{
...
* @return     なし
*/
void PlayerDashParticle::Render(DirectX::SimpleMath::Matrix view, DirectX::SimpleMath::Matrix proj)
{
...
* @return     なし
*/
void PlayerDashParticle::Billboard(DirectX::SimpleMath::Vector3 target, DirectX::SimpleMath::Vector3 eye, DirectX::SimpleMath::Vector3 up)
{
