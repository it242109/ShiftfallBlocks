//--------------------------------------------------------------------------------------
// File: CorrectSwitchParticle.cpp
//
// スイッチパーティクルクラス
//--------------------------------------------------------------------------------------
#include "pch.h"
#include "CorrectSwitchParticle.h"
...
// 定数の定義
const float CorrectSwitchParticle::EMITTER_RANGE_SCALE = 0.2f;			///< エミッタ―の範囲をどれくらいにするか
const int CorrectSwitchParticle::PARTICLE_COUNT = 2;					///< パーティクルの数
const float CorrectSwitchParticle::MIN_SPARN_RADIUS = 0.8f;				///< パーティクルの最小半径
const float CorrectSwitchParticle::MAX_SPARN_RADIUS = 1.0f;				///< パーティクルの最大半径
const float CorrectSwitchParticle::VELOCITY_HORIZONTAL_WEIGHT = 0.3f;	///< 横方向の速度の勢い
const float CorrectSwitchParticle::VELOCITY_UPWARD_BASE = 3.0f;			///< 上方向の基本速度
const float CorrectSwitchParticle::VELOCITY_SPEED_MIN = 3.0f;			///< 最小初速倍率
const float CorrectSwitchParticle::VELOCITY_SPEED_MAX = 1.5f;			///< 最大初速倍率
const float CorrectSwitchParticle::PARTICLE_LIFETIME = 3.0f;			///< 生存時間（秒）
const float CorrectSwitchParticle::PARTICLE_START_SCALE = 1.0f;			///< 開始時のスケール
...
* @return     なし
*/
const std::vector<D3D11_INPUT_ELEMENT_DESC> CorrectSwitchParticle::INPUT_LAYOUT =
{
...
* @return     なし
*/
CorrectSwitchParticle::CorrectSwitchParticle()
	:
	m_pDR(nullptr),
	m_timer(0.0f)
{
...
* @return     なし
*/
void CorrectSwitchParticle::LoadTexture(const wchar_t* path)
{
	// テクスチャリソースの読み込み
...
* @return     なし
*/
void CorrectSwitchParticle::Create(DX::DeviceResources* pDR)
{
	m_pDR = pDR;
	...
	//	画像の読み込み
	LoadTexture(L"Resources/Textures/correctParticle.png");
...
*/
float CorrectSwitchParticle::RandomFloat(float min, float max)
{
...
* @brief　タイプに応じた色を取得
*
* @param[in]  type スイッチの種類
*
* @return    パーティクルの色
*/
DirectX::SimpleMath::Color CorrectSwitchParticle::GetColorFromType(SwitchTargetType type)
{
	// スイッチの種類ごとにパーティクルの色を変える
	switch (type)
	{
	case SwitchTargetType::SW_PLATFORM:	// 足場
		return DirectX::SimpleMath::Color(0.0f, 1.0f, 1.0f, 1.0f);
	case SwitchTargetType::SW_KEY:		// 鍵
		return DirectX::SimpleMath::Color(1.0f, 1.0f, 0.0f, 1.0f);
	case SwitchTargetType::SW_PORTAL:	// ポータル
		return DirectX::SimpleMath::Color(0.8f, 0.0f, 1.0f, 1.0f);
	case SwitchTargetType::SW_ITEM:		// アイテム
		return DirectX::SimpleMath::Color(1.0f, 0.0f, 0.0f, 1.0f);

	case SwitchTargetType::ANY:			// その他／デフォルト
	default:
...
* @return     なし
*/
void CorrectSwitchParticle::Update(float elapsedTime)
{
	// 0.1秒ごとに複数のパーティクルを生成
	m_timer += elapsedTime;
	if (m_timer >= 0.1f)
	{
		...
		for (const auto& emitter : m_emitters)
		{
			...
			for (int i = 0; i < particleCount; ++i)
			{
				...
				float radius = RandomFloat(0.0f, 1.0f); // 0～1の割合
...
* @return     なし
*/
void CorrectSwitchParticle::CreateShader()
{
...
* @return     なし
*/
void CorrectSwitchParticle::Render(DirectX::SimpleMath::Matrix view, DirectX::SimpleMath::Matrix proj)
{
...
* @return     なし
*/
void CorrectSwitchParticle::Billboard(DirectX::SimpleMath::Vector3 target, DirectX::SimpleMath::Vector3 eye, DirectX::SimpleMath::Vector3 up)
{
